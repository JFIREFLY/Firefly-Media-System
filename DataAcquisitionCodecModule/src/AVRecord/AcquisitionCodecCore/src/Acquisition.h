...
#include "./include/AcquisitionCodec.h"
#include <QVariant>
#include <chrono>
#include <memory>
...
class Acquisition : public AcquisitionCodec
{
public:
    Acquisition(const QVariantMap& recordInfo);
    ~Acquisition();

    void setRecordInfo(const QVariantMap& recordInfo) override;
    int  startRecord(bool b) override;
    int  pauseRecord() override;
    int  resumeRecord() override;
    int  stopRecord() override;

private:
    void startCapture(bool b);
	void stopCapture();

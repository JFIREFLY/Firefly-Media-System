...
#else
    //编码质量和速度
    av_opt_set(&m_dict, "preset", "ultrafast", 0);
    av_opt_set(&m_dict, "tune", "zerolatency", 0);
    av_opt_set(&m_dict, "crf", "23", 0);
#endif
...
int AudioEncoder::encode(AVFrame* frame, int stream_index, int64_t pts, int64_t time_base, std::vector<AVPacket*>& packets)
{
    ...
    while (true)
    {
        ...
        #if  AUDIO
        qCritical() << "(avcodec_receive_packet)[AudioEncoder]packet.size: " << packet->size;
        #endif        

...
int VideoCapture::initCapture()
{
    ...
    av_dict_set(&options, "fflags", "nobuffer", 0);  //零延迟
#endif

    //自定义查找当前设备存在的合法dshow设备
    // 获取摄像头列表
    ...
    {
        qCritical() << "cameraInfo[" << id << "]:\n"
                 << "VideoDriveInfo:" + cameraInfo.deviceName() << "\t";
        cameraName.insert(id, cameraInfo.description());
        qCritical() << "description:" + cameraName[id] << "\n";
        id++;
    }
    #if 0
    QString urlString;
    if (cameraName.size() > 1)
        urlString = QString("video=") + cameraName[1];
    else
        urlString = QString("video=") + cameraName[0];     
    if ((ret = avformat_open_input(&m_vFmtCtx, urlString.toLocal8Bit().data(), ifmt, &options)) < 0)
    #else
...
#if VIDEO
        qCritical() << "(av_read_frame)[videoCaptureThreadProc]pkt.size: " << pkt.size;
#endif
